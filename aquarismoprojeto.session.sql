CREATE DATABASE IF NOT EXISTS aquarismo;
USE aquarismo;
...
CREATE TABLE produto (
    ...
    quantidade INT,
    descricao TEXT,
    valor DECIMAL(10,2)--10 digitos sendo 8 antes da virgula e 2 dps evita erro de arredondamento como aconteceria com float e double
);
...
DELIMITER //--"//" vira o novo delimitador ja que o mysql le originalemnte o ";" como o fim da instruçao dando erro 
...
BEGIN
  ...
  IF total_vendas > 1000 THEN
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = CONCAT('Total bônus necessário: R$', bonus_total);
  END IF;
END //
DELIMITER ;

--cliente que comprou > R$500 entra como cliente especial e recebe 2% de cashback
DELIMITER //
...
BEGIN
  ...
  SELECT v.id_cliente INTO cid FROM venda v WHERE v.id = NEW.id_venda;
  ...
  IF total_gasto > 500 THEN
    ...
    INSERT IGNORE INTO clienteespecial (nome, sexo, idade, id_cliente, cashback)
    SELECT c.nome, c.sexo, c.idade, c.id, cashback_valor FROM cliente c WHERE c.id = cid;

    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = CONCAT('Total cashback necessário: R$', cashback_valor);
  END IF;
END //
DELIMITER ;

--Se cashback de cliente especial for zerado ele e removido da tabela
DELIMITER //
...
--view
--produtos vendidos com total vendido por produto
...
--clientes especiais com total gasto
